...
function App() {
  ...
  const handleLaunchScrcpy = async (serial, force = false) => {
    addLog(`${force ? 'Restarting' : 'Launching'} scrcpy for ${serial}...`, 'info');
    try {
      await invoke('launch_scrcpy', { serial, force });
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {adbDevices.map(device => (
                    <div key={device.serial} className="bg-app-surface border border-app-border rounded-xl p-4 hover:border-app-primary/50 transition-all group shadow-sm hover:shadow-app-primary/10">
                      ...
                      <button
                        onClick={() => handleLaunchScrcpy(device.serial, false)}
                        className="flex-1 flex items-center justify-center gap-2 bg-app-bg hover:bg-zinc-700 text-app-text py-2 rounded-lg text-sm font-medium transition-colors hover:text-white"
                      >
                        <Play size={16} className="fill-current" /> {t.mirror}
                      </button>
                      <button
                        onClick={() => handleLaunchScrcpy(device.serial, true)}
                        title="Force New Session"
                        className="flex items-center justify-center px-3 bg-app-bg hover:bg-red-500/20 text-app-text-muted hover:text-red-500 border border-transparent hover:border-red-500/50 rounded-lg transition-all"
                      >
                        <RefreshCw size={16} />
                      </button>
                    </div>
